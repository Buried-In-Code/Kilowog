...
class TeamTest {
    ...
    inner class ListTeams {
        ...
        fun `Test ListTeams with a valid search`() {
            ...
            assertAll(
                { assertEquals(1473, results[0].id) },
                { assertEquals("Rat Creatures", results[0].name) },
            )
        }
    ...
    inner class GetTeam {
        ...
        fun `Test GetTeam with a valid id`() {
            ...
            assertAll(
                ...
                { assertEquals("https://static.metron.cloud/media/team/2024/03/07/f957fc534c0245abafbecb5e8bb4dafa.jpg", result.image) },
