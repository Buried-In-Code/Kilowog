...
class ArcTest {
    ...
    inner class ListArcs {
        ...
        fun `Test ListArcs with a valid search`() {
            val results = session.listArcs(params = mapOf("name" to "Bone"))
            assertEquals(1, results.size)
            assertAll(
                { assertEquals(1491, results[0].id) },
                { assertEquals("\"Bone\" The Great Cow Race", results[0].name) },
            )
        }
    ...
    inner class GetArc {
        ...
        fun `Test GetArc with a valid id`() {
            ...
            assertAll(
                ...
                { assertEquals("https://static.metron.cloud/media/arc/2024/03/07/d75aba2ca26349c89c3104690d32cc2f.jpg", result.image) },
                { assertEquals("\"Bone\" The Great Cow Race", result.name) },
