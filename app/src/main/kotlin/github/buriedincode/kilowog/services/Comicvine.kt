...
import github.buriedincode.comicvine.SQLiteCache
import github.buriedincode.kilowog.Settings
import github.buriedincode.kilowog.Utils
import io.github.oshai.kotlinlogging.KotlinLogging
import kotlin.io.path.div
import github.buriedincode.comicvine.Comicvine as Session

class Comicvine(settings: Settings.Comicvine) {
    private val session = Session(
        apiKey = settings.apiKey!!.toString(),
        cache = SQLiteCache(path = (Utils.CACHE_ROOT / "comicvine.sqlite"), expiry = 14),
    )
    ...
    private fun addPublisher(publisher: Publisher, metronInfo: MetronInfo) {
    ...
    private fun addPublisher(publisher: Publisher, comicInfo: ComicInfo) {
    ...
    private fun addVolume(volume: Volume, metronInfo: MetronInfo) {
        if (metronInfo.id == null || metronInfo.id!!.source == InformationSource.COMIC_VINE) {
    ...
    private fun addVolume(volume: Volume, comicInfo: ComicInfo) {
    ...
    private fun searchIssues(volumeId: Long, number: String?): Long? {
    ...
    private fun addIssue(issue: Issue, metadata: Metadata) {
    ...
    private fun addIssue(issue: Issue, metronInfo: MetronInfo) {
    ...
    private fun addIssue(issue: Issue, comicInfo: ComicInfo) {
    ...
    override fun fetch(details: Details): Triple<Metadata?, MetronInfo?, ComicInfo?> {
        if (details.series.comicvine == null && details.issue.comicvine != null) {
            try {
                this.session.getIssue(id = details.issue.comicvine!!).also {
                    details.series.comicvine = it.volume.id
                }
